...
function linearize_dynamics(x̅::AbstractMatrix{T}, u̅::AbstractMatrix{T},
    ...
    N, control_size = size(u̅); state_size = size(x̅)[2];
...
function cost_quadratization(x̅::AbstractMatrix{T}, u̅::AbstractMatrix{T},
                             immediate_cost::Function, final_cost::Function) where {T}
    N, control_size = size(u̅); state_size = size(x̅)[2];
...
@doc raw"""
"""
function optimal_controller_param(𝐀ᵢ::AbstractMatrix{T},
                                  𝐁ᵢ::AbstractMatrix{T},
                                  𝐫ᵢ::AbstractMatrix{T},
                                  𝐏ᵢ::AbstractMatrix{T},
                                  𝐑ᵢ::AbstractMatrix{T},
...
@doc raw"""
"""
function feedback_parameters(𝐠ᵢ::AbstractMatrix{T},
                             𝐆ᵢ::AbstractMatrix{T},
                             𝐇ᵢ::AbstractMatrix{T}
                             ) where {T}
    𝛿𝐮ᵢᶠᶠ = - inv(𝐇ᵢ) * 𝐠ᵢ
...
```
"""
function backward_pass(x̅::AbstractMatrix{T},
                       u̅::AbstractMatrix{T},
                       dynamicsf::Function,
    ...
    (𝑞s, 𝐪s, 𝐫s, 𝐐s, 𝐏s, 𝐑s) = cost_quadratization(x̅, u̅,
                                                   immediate_cost, final_cost)
    # Grab all dimensions
    ...
    # Move backward
    for i = N:1
        (𝛿𝐮ᵢᶠᶠ, 𝐊ᵢ, 𝑠ᵢ, 𝐬ᵢ, 𝐒ᵢ) = back_one_step(𝐀s[i], 𝐁s[i], 𝑞s[i], 𝐪s[i],
                                               𝐫s[i], 𝐐s[i], 𝐏s[i], 𝐑s[i],
                                               𝑠ᵢ₊₁, 𝐬ᵢ₊₁, 𝐒ᵢ₊₁)
        𝛿𝐮ᶠᶠs[i] .= 𝛿𝐮ᵢᶠᶠ
        𝐊s[i] .= 𝐊ᵢ
