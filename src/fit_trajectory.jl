...
function fit(x_init::AbstractMatrix{T}, u_init::AbstractMatrix{T},
             ...
             final_cost::Function; max_iter::Int64=100, tol::Float64=1e-6) where T
    x̅ⁱ = x_init; u̅ⁱ = u_init;
    N, input_size = size(u̅ⁱ); M, state_size = size(x̅ⁱ);
    @assert(N + 1 == M, "size(x_init)[2] == size(u_init)[1], (# of states is 1 more than # of inputs in trajectory)")
